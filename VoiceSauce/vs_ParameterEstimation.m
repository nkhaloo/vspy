...
BandwidthMethod = 'formula';

if (isempty(strfind(VSData.vars.BandwidthMethod, 'formula')))
    BandwidthMethod = 'estimated';
end
...
for k=1:length(filelist)
    ...
    [y, Fs, nbits] = func_readwav(wavfile);
    ...
    if (get(handles.checkbox_process16khz, 'Value') == 1)
        if (Fs > 16000)
            ...
            func_writewav(y, 16000, nbits, wavfile);
            warning on;
            resampled = 1;
        end
        [y, Fs] = func_readwav(wavfile);  % reread the resampled file
    end
    ...
    for n=1:length(paramlist)
        ...
        if (exist(matfile, 'file'))
            save(matfile, 'windowsize', 'frameshift', '-append');
        else
            save(matfile, 'windowsize', 'frameshift');
        end
    ...
    % F0 (Snack)
        elseif (strcmp(paramlist{n}, 'F0 (Snack)'))
            ...
            if (nbits ~= 16)
                ...
                use_alt_file = 1;
                func_writewav(y, Fs, 16, snackwavfile);
            else
    ...
    % F0 (SHR)
        elseif (strcmp(paramlist{n}, 'F0 (SHR)'))
            ...
            if (exist(matfile, 'file'))
                save(matfile, 'shrF0', 'Fs', '-append');
            else
                save(matfile, 'shrF0', 'Fs');
            end
    ...
    % Formants (Snack)
        elseif (strcmp(paramlist{n}, 'Formants (Snack)'))
            ...
            [sF1, sF2, sF3, sF4, sB1, sB2, sB3, sB4, err] = func_SnackFormants(snackwavfile, windowsize/1000, frameshift/1000, preemphasis, lpcOrder);
            
            if (strcmp(VSData.vars.FMTalgorithm, 'Formants (Snack)') && err == 1)
                messages{k+1} = 'Error: Problem with snack - please check settings';
                set(MBoxHandles.listbox_messages, 'String', messages, 'Value', k+1);
                set(MBoxHandles.pushbutton_close, 'Enable', 'on');
                set(MBoxHandles.pushbutton_stop, 'Enable', 'off');
    ...
    % Formants (Praat)
        elseif (strcmp(paramlist{n}, 'Formants (Praat)'))
            messages{k} = [messages{k} 'FMTp '];
            set(MBoxHandles.listbox_messages, 'String', messages, 'Value', k);
            drawnow; 
                        
            num_praat_formants = VSData.vars.FormantsPraatNumFormants;
            max_formant_freq = VSData.vars.FormantsPraatMaxFormantFreq;
            
            [pF1, pF2, pF3, pF4, pF5, pF6, pF7, pB1, pB2, pB3, pB4, pB5, pB6, pB7, err] = ...
                func_PraatFormants(wavfile, windowsize/1000, frameshift/1000, frame_precision, data_len, num_praat_formants, max_formant_freq);
            
            if (strcmp(VSData.vars.FMTalgorithm, 'Formants (Praat)') && err ~= 0)
                messages{k+1} = 'Error: Problem with Praat - please check settings';
                set(MBoxHandles.listbox_messages, 'String', messages, 'Value', k+1);
                set(MBoxHandles.pushbutton_close, 'Enable', 'on');
                set(MBoxHandles.pushbutton_stop, 'Enable', 'off');
            ...
            if (num_praat_formants >= 5)
                save(matfile, 'pF5', 'pB5', '-append');
            end
            if (num_praat_formants >= 6)
                save(matfile, 'pF6', 'pB6', '-append');
            end
            if (num_praat_formants >= 7)
                save(matfile, 'pF7', 'pB7', '-append');
            end
            
    % Formants (Other)
        elseif (strcmp(paramlist{n}, 'Formants (Other)'))
            ...
            err = 0;
            try 
                [F1, F2, F3, F4, B1, B2, B3, B4, errmsg] = ...
                    func_OtherFormants(wavfile, handles);
            catch
                err = 1;
            end
    ...
    % 2K
        elseif (strcmp(paramlist{n}, '2K'))
            messages{k} = [messages{k} '2K '];
            set(MBoxHandles.listbox_messages, 'String', messages, 'Value', k);
            drawnow;
            ...
            if (useTextgrid)
                [H2K, F2K, isComplete] = func_Get2K(y, Fs, F0, MBoxHandles, VSData.vars, textgridfile);
            else
                [H2K, F2K, isComplete] = func_Get2K(y, Fs, F0, MBoxHandles, VSData.vars);
            end
                        
            H2KFMTalgorithm = FMTalgorithm;
            
            save(matfile, 'H2K', 'F2K', 'H2KFMTalgorithm', '-append');
            
            
    % 5K
        elseif (strcmp(paramlist{n}, '5K'))
            messages{k} = [messages{k} '5K '];
            set(MBoxHandles.listbox_messages, 'String', messages, 'Value', k);
            drawnow;
            ...
            if (useTextgrid)
                [H5K, isComplete] = func_Get5K(y, Fs, F0, MBoxHandles, VSData.vars, textgridfile);
            else
                [H5K, isComplete] = func_Get5K(y, Fs, F0, MBoxHandles, VSData.vars);
            end
    ...
    % H1*-H2* and H2*-H4*
        elseif (strcmp(paramlist{n}, 'H1*-H2*, H2*-H4*'))
            ...
            matdata = load(matfile);
            F0 = func_parseF0(matdata, F0algorithm);
            [F1, F2, F3, B1, B2, B3] = func_parseFMT(matdata, FMTalgorithm);
            
            if (strcmp(BandwidthMethod, 'formula'))
                [H1H2c, H2H4c] = func_GetH1H2_H2H4(matdata.H1, matdata.H2, matdata.H4, Fs, F0, F1, F2);
            else
                [H1H2c, H2H4c] = func_GetH1H2_H2H4(matdata.H1, matdata.H2, matdata.H4, Fs, F0, F1, F2, B1, B2);
            end
            save(matfile, 'H1H2c', 'H2H4c', 'BandwidthMethod', '-append');
    ...
    % H1*-A1*, H1*-A2*, and H1*-A3*
        elseif (strcmp(paramlist{n}, 'H1*-A1*, H1*-A2*, H1*-A3*'))
            ...
            if (strcmp(BandwidthMethod, 'formula'))
                ...
                [H1A1c, H1A2c, H1A3c] = func_GetH1A1_H1A2_H1A3(matdata.H1, matdata.A1, matdata.A2, matdata.A3, Fs, F0, F1, F2, F3, B1, B2, B3);
            end
            save(matfile, 'H1A1c', 'H1A2c', 'H1A3c', 'BandwidthMethod', '-append');
                        

    % H4*-2K*
        elseif (strcmp(paramlist{n}, 'H4*-2K*'))
            messages{k} = [messages{k} 'H42Kc '];
            set(MBoxHandles.listbox_messages, 'String', messages, 'Value', k);
            drawnow;
            ...
            if (strcmp(BandwidthMethod, 'formula'))
                [H42Kc] = func_GetH42K(matdata.H4, matdata.H2K, matdata.F2K, Fs, F0, F1, F2, F3);
            else
                [H42Kc] = func_GetH42K(matdata.H4, matdata.H2K, matdata.F2K, Fs, F0, F1, F2, F3, B1, B2, B3);
            end
            save(matfile, 'H42Kc', 'BandwidthMethod', '-append');
            
            
    % 2K*-5K
        elseif (strcmp(paramlist{n}, '2K*-5K'))
            messages{k} = [messages{k} '2K5Kc '];
            set(MBoxHandles.listbox_messages, 'String', messages, 'Value', k);
            drawnow;
            ...
            if (strcmp(BandwidthMethod, 'formula'))
                [H2KH5Kc] = func_Get2K5K(matdata.H2K, matdata.F2K, matdata.H5K, Fs, F0, F1, F2, F3);
            else
                [H2KH5Kc] = func_Get2K5K(matdata.H2K, matdata.F2K, matdata.H5K, Fs, F0, F1, F2, F3, B1, B2, B3);
            end
            save(matfile, 'H2KH5Kc', 'BandwidthMethod', '-append');
    ...
    % SHR
     ...
     % epoch and SoE
        elseif (strcmp(paramlist{n}, 'epoch, excitation strength (epoch, SoE)'))
            messages{k} = [messages{k} 'epoch and SoE '];
            set(MBoxHandles.listbox_messages, 'String', messages, 'Value', k);
            drawnow;
            ...
            try
                METHOD = 'whole';
                [epoch, soe, z]=func_getSoE(y, data_len, Fs, METHOD, F0, VSData.vars);
            catch
                err = 1; 
            end
